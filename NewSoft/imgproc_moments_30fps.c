...
#define DEFAULT_BASELINE_MM 100
...
{
    fprintf(stderr,
            "usage: %s [threshold_a [threshold_b [focal_x focal_y baseline_mm]]]\n",
            name);
    fprintf(stderr, "  thresholds: 0..255\n");
    fprintf(stderr, "  focal_x/focal_y: focal lengths in pixels\n");
    fprintf(stderr, "  baseline_mm: distance between camera centers in millimeters\n");
}
...
int main(int argc, char **argv)
{
    ...
    unsigned baseline_mm = DEFAULT_BASELINE_MM;
    ...
    if (argc >= 6 && parse_u32_limit(argv[5], 100000u, &baseline_mm) < 0) {
        usage(argv[0]);
        return 2;
    }

    if (focal_x == 0 || focal_y == 0 || baseline_mm == 0) {
        fprintf(stderr, "focal_x, focal_y, and baseline_mm must be nonzero\n");
        return 2;
    }
    ...
    printf("frame store off, thresholdA=%u thresholdB=%u focalX=%u focalY=%u baselineMM=%u CONTROL=0x%08x\n",
           threshold_a, threshold_b, focal_x, focal_y, baseline_mm,
           regs[IMG_CONTROL]);
    ...
    printf("areaA,uA,vA,xA,yA,angleXA,angleYA,areaB,uB,vB,xB,yB,angleXB,angleYB,valid3D,xMM,yMM,zMM,done,hz\n");
    ...
    while (1) {
        ...
        if (sample % PRINT_EVERY == 0) {
            ...
            if (area_a != 0 && area_b != 0) {
                int32_t dx_a = (int32_t)x_a - CENTER_X;
                int32_t dx_b = (int32_t)x_b - CENTER_X;
                int32_t dy_a = CENTER_Y - (int32_t)y_a;
                int32_t dy_b = CENTER_Y - (int32_t)y_b;
                int32_t disparity = dx_a - dx_b;

                if (disparity > 0) {
                    int64_t z = ((int64_t)baseline_mm * (int64_t)focal_x) /
                                (int64_t)disparity;

                    int64_t x = -((int64_t)baseline_mm / 2) +
                                (z * dx_a) / (int64_t)focal_x;

                    int64_t y_a_mm = (z * dy_a) / (int64_t)focal_y;
                    int64_t y_b_mm = (z * dy_b) / (int64_t)focal_y;
                    int64_t y = (y_a_mm + y_b_mm) / 2;
            ...
            printf(",%d,%" PRId32 ",%" PRId32 ",%" PRId32 ",0x%08x,%.2f\n",
                   valid_3d, x_mm, y_mm, z_mm, done_snapshot, hz);
